...
async function getItems() {
  ...
  for (let i = 0; i < res.items.length; i++) {
    ...
    let totalprice = 0
    if (per > 0) {      
      totalprice = util.getInt((util.getFloat(data.report.saveprice)) * parseFloat(util.getPlanyears(data.repair.planyears)) / 100 * per)
    }
  ...
  let compares = [
    {
      ...
      title2: '총 적립금액',
      price: util.money(plan.total),
      price2: util.money(Math.round(util.getFloat(plan.area) * util.getFloat(data.repair.savingprice) * 12 * util.getPlanyears(data.repair.planyears)))
    },
    {
      index: 2,
      title: '기준일까지 계획금액',
      title2: '기준일까지 적립금액',
      price: util.money(Math.round(util.getInt(plan.reportyear) - util.getInt(plan.startyear) + 1) * util.getFloat(plan.total) / years * 12),
      price2: util.money(Math.round(util.getFloat(plan.area) * util.getFloat(data.repair.savingprice) * 12 * util.getFloat(util.getInt(plan.reportyear) - util.getInt(plan.startyear) + 1)))
    },
    {
      index: 3,
      title: '년 평균 계획금액',
      title2: '년 평균 적립금액',
      price: util.money(Math.round(util.getFloat(plan.total) / years * 12)),
      price2: util.money(Math.round(util.getFloat(plan.area) * util.getFloat(data.repair.savingprice) * 12))
    },
    {
      index: 4,
      title: '월 평균 계획금액',
      title2: '월 평균 적립금액',
      price: util.money(Math.round(util.getFloat(plan.total) / years)),
      price2: util.money(Math.round(util.getFloat(plan.area) * util.getFloat(data.repair.savingprice)))
    },
    {
      index: 5,
      title: 'm2 평균 계획금액',
...
function makeItems(items) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    //let price = parseFloat(totalprice) / (util.getFloat(data.report.totalsize) * 12.0 * (util.getFloat(item.endyear) - util.getFloat(item.startyear) + 1))
...
async function clickBatch() {
  ...
  /*
    ...
    for (let i = 0; i <= items.length - 1; i++) {
      ...
      let price = parseFloat(totalprice) / (util.getFloat(data.report.totalsize) * duration)  
      items[i].totalprice = util.money(totalprice)    
      items[i].price = util.moneyfloat(price)
    }
  */
  }    
...
function clickAdd() {
  ...
  for (let i = 0; i < data.batchs.length; i++) {
    ...
    data.batchs[i].totalrate = util.getFloat(sum)
  }
}
...
function clickAddPlan() {
  ...
  for (let i = 0; i < data.batchplans.length; i++) {
    ...
    data.batchplans[i].totalrate = util.getFloat(sum)
  }
}
...
function onKeyup(index, col) {
  if (col == 1) {
    ...
    data.batchs[index].duration = getDurationMonth(data.batchs[index].endyear, data.batchs[index].endmonth, data.batchs[index].startyeardata.batchs[index].startmonth)
  } else if (col == 2) {
    ...
    let duration = util.getFloat(getDurationMonth(item.endyear, item.endmonth, item.startyear, item.startmonth))
    let per = (util.getFloat(data.report.totalsize) * duration) * util.getFloat(item.price) / (util.getFloat(data.report.saveprice) * parseFloat(util.getPlanyears(data.repair.planyears))) * 100

    let totalprice = util.getInt((util.getFloat(data.report.saveprice)) * parseFloat(util.getPlanyears(data.repair.planyears)) / 100 * per)
    data.batchs[index].rate = util.getFloat(per)
    data.batchs[index].totalprice = util.money(totalprice)
  }
...
function onKeyupPlan(index, col) {
  if (col == 1) {
    ...
    let totalprice = util.getInt((util.getFloat(data.report.saveprice)) * parseFloat(util.getPlanyears(data.repair.planyears)) / 100 * per)
    data.batchplans[index].rate = util.getFloat(per)
    data.batchplans[index].totalprice = util.money(totalprice)    
  }
...
function clickHundred() {
  ...
  data.batchs[index].rate = (100.0 - total)
...
function clickHundredPlan() {
  ...
  data.batchplans[index].rate = (100.0 - total)
