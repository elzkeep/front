...
async function getItems() {
  ...
  if (res.items != null) {   
    for (let i = 0; i < res.items.length; i++) {
      ...
      let total = util.getInt(item.forward) + util.getInt(item.interest) + util.getInt(item.surplus) + util.getInt(item.saving) - util.getInt(item.use)
