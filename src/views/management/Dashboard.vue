<template>
  <el-row style="margin:-10px 20px 20px 20px;">
    ...
    <el-col :span="9">
      ...
      <PageTitle title="기본 정보" />

      <y-table>
        <y-tr>
          <y-th style="width:60px;">회사명</y-th>
          <y-td>{{data.company.name}}</y-td>
        </y-tr>        
        <y-tr>
          <y-th>연락처</y-th>
          <y-td>{{data.company.tel}}</y-td>
        </y-tr>
        <y-tr>
          <y-th>이메일</y-th>
          <y-td>{{data.company.email}}</y-td>
        </y-tr>
        <y-tr>
          <y-th style="height:50px;">청구정보</y-th>
          <y-td>

            <span v-bind:style="{color: checkLimitdate(data.company.limitdate) ? '' : 'red'}">{{util.viewDate(data.company.limitdate)}}</span>
            <!--
            &nbsp;&nbsp;
            ...
            >
              <template #reference>
                <el-button size="small" >연장</el-button>
              </template>

              <el-alert title="사용기간 만료전에 연장을 해야 계속 사용하실 수 있습니다" type="error" :closable="false" />

              <div style="margin-top:10px;">연장금액 : 500,000원</div>
    ...
    <el-col :span="6">
      <div style="margin-left:20px;">
        ...
        <y-table style="height:145px;">
          <y-tr>
            <y-td>              
              <div style="font-weight:bold;font-size:30px;margin: 0px auto;text-align:center;"><span style="color:#f56c6c;">{{data.buildingscore}}</span> / {{data.userscore}}</div>
            </y-td>
          </y-tr>
        </y-table>
      </div>
