<template>
  ...
  <el-table :data="data.items" border style="width: 100%;" :span-method="spanMethod"  :cell-class-name="cellClassName" :height="height(200)" v-infinite="getItems" @row-click="clickUpdate">
    ...
    <el-table-column label="정밀점검" width="80" align="center" >
      <template #default="scope">
        <el-button size="small" v-if="scope.row.contracttype & 2" type="primary" @click="clickDetail(scope.row)">정밀점검</el-button>
      </template>
    </el-table-column>
    <el-table-column label="정기점검" width="80" align="center" >
      <template #default="scope">
        <el-button size="small" v-if="scope.row.contracttype & 4" type="info" @click="clickPeriodic(scope.row)">정기점검</el-button>
      </template>
    </el-table-column>
