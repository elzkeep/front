<template>
  ...
  <el-table :data="data.items" border :height="height(170)" @row-click="clickUpdate"  ref="listRef" @selection-change="changeList">
    ...
    <el-table-column label="빌딩" align="left" width="200">
      <template #default="scope">
        {{getBuilding(scope.row.building)}}
      </template>
    </el-table-column>
    <el-table-column label="관리형태" align="center" width="100">
      <template #default="scope">
        <span v-if="scope.row.type==1">직영</span>
        <span v-if="scope.row.type==2">위탁관리</span>
      </template>
    </el-table-column>
    <el-table-column prop="managername" label="담당자" align="left" />
...
import { Customer, Building, Company } from "~/models"
