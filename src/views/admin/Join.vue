  ...
  <el-table :data="data.items" border :height="height(170)" @row-click="clickUpdate"  ref="listRef" @selection-change="changeList">
    ...
    <el-table-column label="구분" width="60" align="center">
      <template #default="scope">
        <span v-if="scope.row.category == 1">안전관리대행사업자</span>
        <span v-if="scope.row.category == 2">상주관리자</span>
        <span v-if="scope.row.category == 3">기업/시설팀</span>
        <span v-if="scope.row.category == 4">기타</span>
      </template>
    </el-table-column>
  ...
  >
      ...
      <y-table>
        <y-tr>
          ...
          <y-td>
            <el-radio-group v-model="data.item.category">
              <el-radio-button size="small" value="1">안전관리대행사업자</el-radio-button>
              <el-radio-button size="small" value="2">상주관리자</el-radio-button>
              <el-radio-button size="small" value="3">기업/시설팀</el-radio-button>
              <el-radio-button size="small" value="4">기타</el-radio-button>
            </el-radio-group>
          </y-td>
        </y-tr>
        <y-tr>
        ...
        <y-tr>
          <y-th>연락담당자</y-th>
          <y-td>
            <el-input v-model="data.item.manager" />
          </y-td>
        </y-tr>
        <y-tr>
          <y-th>연락처</y-th>
