      ...
      <div style="flex:1;">
        ...
        <y-table>
         ...
         <div style="margin-top:10px;">
           ...
           <el-button style="display:block;float:left;" size="small" type="success" @click="clickContractInsert">계약 등록</el-button>
         ...
         <el-tabs v-model="data.menu" @tab-click="clickTab">
           ...
           <el-tab-pane label="견적 내역" name="estimate">
             <el-table :data="data.estimates" border :height="height(540)" @row-click="clickEstimate">
               <el-table-column prop="index" label="NO" align="center" width="60" />
               <el-table-column prop="type" label="구분" align="center" width="130">
                 <template #default="scope">
               ...
               <el-table-column label="견적일" align="center" width="120">
                 <template #default="scope">
               ...
               <el-table-column align="center" width="140">
                 <template #default="scope">
                   ...
                   <el-button size="small" type="primary" @click="clickDownloadEstimate(scope.row)" style="margin-left:5px;">다운로드</el-button>                   
                 </template>
               </el-table-column>
             </el-table>
           </el-tab-pane>
           ...
           <el-tab-pane label="계약 내역" name="contract">
             <el-table :data="data.contracts" border :height="height(540)" @row-click="clickContract">
               ...
               <el-table-column label="계약일" align="center" width="120">
                 <template #default="scope">
                   {{util.viewDate(scope.row.contractdate)}}
                 </template>
               </el-table-column>
               <el-table-column label="계약기간" align="center" width="250">
                 <template #default="scope">
               ...
               <el-table-column label="VAT" align="center" width="50">
                 <template #default="scope">
                   <span v-if="scope.row.vat == 1">포함</span>
                   <span v-if="scope.row.vat == 2">별도</span>
                 </template>
               </el-table-column>
               <el-table-column label="세금계산서발행" align="center" width="120">
                 <template #default="scope">
                   {{util.viewDate(scope.row.invoice)}}
                 </template>
               </el-table-column>
               <el-table-column label="입금일" align="center" width="120">
                 <template #default="scope">
...
<el-dialog
  ...
  >
    ...
    <y-table>
      <y-tr>
        <y-th style="width:60px;text-align:center;">구분</y-th>
        <y-td>
          <el-radio-group v-model.number="data.estimate.type">
            <el-radio-button size="small" label="1" @click="clickType(1)">장기수선</el-radio-button>
            <el-radio-button size="small" label="2" @click="clickType(2)">정밀점검</el-radio-button>
            <el-radio-button size="small" label="3" @click="clickType(3)">정기점검</el-radio-button>
          </el-radio-group>

          <el-radio-group v-model.number="data.estimate.subtype"  v-if="data.estimate.type == 1" style="margin-left:20px;">
          ...
          <el-radio-group v-model.number="data.estimate.subtype"  v-if="data.estimate.type == 3" style="margin-left:20px;">
            ...
            <el-radio-button size="small" label="4" @click="clickSubtype(4)">정기 5회</el-radio-button>
          </el-radio-group>

          <el-select v-model="data.estimate.start" style="margin-left:20px;" placeholder=""  v-if="(data.estimate.type == 3 && data.estimate.subtype == 4) || data.estimate.type == 2">
            <el-option
      ...
      <y-tr>
        <y-th style="text-align:center;">견적일</y-th>
        <y-td>
          <el-date-picker style="margin: 0px 0px;height: 24px;width:120px;" v-model="data.estimate.writedate" placeholder="" />
        </y-td>
      </y-tr>
      <y-tr>
        <y-th style="text-align:center;">비용</y-th>
        <y-td>
          <div :style="{height: height(480), overflow: 'auto'}">
          <y-table>
            <y-tr>
              <y-th rowspan="2" style="width:120px;">직접 인건비</y-th>              
              <y-th rowspan="2" style="width:100px;text-align:right;">단가</y-th>
              <y-th colspan="2" style="text-align:center;">수량</y-th>              
              <y-th rowspan="2" style="text-align:right;padding-right:20px;">계</y-th>
            </y-tr>

            <y-tr>              
              <y-th style="text-align:right;">내업</y-th>
              <y-th style="text-align:right;">외업</y-th>              
            </y-tr>
            ...
            <y-tr>
              <y-th>특급기술자</y-th>
              <y-td style="width:120px;text-align:right;">{{util.money(data.standardwage.person2)}}</y-td>
              <y-td style="text-align:right;">
                <el-input v-model.number="data.estimate.person2" style="width:70px;" @input="changePrice" />
              </y-td>
              <y-td style="text-align:right;">
                <el-input v-model.number="data.estimate.person7" style="width:70px;" @input="changePrice" v-if="data.estimate.type != 1" />
              </y-td>              
              <y-td style="width:120px;text-align:right;padding-right:20px;">{{util.money(calc.price2)}}</y-td>              
            </y-tr>
            <y-tr>
              <y-th>고급기술자</y-th>
              <y-td style="width:120px;text-align:right;">{{util.money(data.standardwage.person3)}}</y-td>
              <y-td style="text-align:right;">
                <el-input v-model.number="data.estimate.person3" style="width:70px;" @input="changePrice" />
              </y-td>
              <y-td style="text-align:right;">
                <el-input v-model.number="data.estimate.person8" style="width:70px;" @input="changePrice" v-if="data.estimate.type != 1" />
              </y-td>              
              <y-td style="text-align:right;padding-right:20px;">{{util.money(calc.price3)}}</y-td>              
            </y-tr>
            <y-tr>              
              <y-th>중급기술자</y-th>
              <y-td style="width:120px;text-align:right;">{{util.money(data.standardwage.person4)}}</y-td>
              <y-td style="text-align:right;">
                <el-input v-model.number="data.estimate.person4" style="width:70px;" @input="changePrice" />
              </y-td>
              <y-td style="text-align:right;">
                <el-input v-model.number="data.estimate.person9" style="width:70px;" @input="changePrice" v-if="data.estimate.type != 1" />
              </y-td>              
              <y-td style="text-align:right;padding-right:20px;">{{util.money(calc.price4)}}</y-td>
            </y-tr>
            <y-tr>
              <y-th>초급기술자</y-th>
              <y-td style="width:120px;text-align:right;">{{util.money(data.standardwage.person5)}}</y-td>
              <y-td style="text-align:right;">
                <el-input v-model.number="data.estimate.person5" style="width:70px;" @input="changePrice" />
              </y-td>
              <y-td style="text-align:right;">
                <el-input v-model.number="data.estimate.person10" style="width:70px;" @input="changePrice" v-if="data.estimate.type != 1" />
              </y-td>              
              <y-td style="text-align:right;padding-right:20px;">{{util.money(calc.price5)}}</y-td>
            </y-tr>
            <y-tr>
              <y-th>계</y-th>
              <y-td></y-td>
              <y-td></y-td>
              <y-td></y-td>
              <y-td style="text-align:right;padding-right:20px;">{{util.money(calc.price)}}</y-td>
            </y-tr>
            <y-tr>
              <y-th>제경비</y-th>
              <y-td colspan="3">
                직접인건비 ✕ <el-input v-model="data.estimate.financialprice" class="inputNumber" style="width:70px;" @input="changePrice" /> %
              </y-td>              
              <y-td style="text-align:right;padding-right:20px;">{{util.money(calc.financialprice)}}</y-td>
            </y-tr>
            <y-tr>
              <y-th>기술료</y-th>              
              <y-td colspan="3">
                (직접인건비 + 제경비) ✕ <el-input v-model="data.estimate.techprice" class="inputNumber" style="width:70px;" @input="changePrice" /> %
              </y-td>
              <y-td style="text-align:right;padding-right:20px;">{{util.money(calc.techprice)}}</y-td>
            </y-tr>
            ...
            <y-tr v-if="data.estimate.type != 1">
              <y-th>직접경비</y-th>
              <y-td colspan="3"></y-td>              
              <y-td style="text-align:right;padding-right:20px;">{{util.money(calc.directprice)}}</y-td>
            </y-tr>
            <y-tr v-if="data.estimate.type != 1">
              <y-th>&nbsp;&nbsp;&nbsp;여비 및 현장체재비</y-th>
              ...
              <y-td style="text-align:right;padding-right:20px;">{{util.money(calc.travelprice)}}</y-td>
            </y-tr>            
            <y-tr v-if="data.estimate.type != 1">
              <y-th>&nbsp;&nbsp;&nbsp;차량운행비</y-th>              
              <y-td style="text-align:right;padding-right:10px;">
              ...
              <y-td style="text-align:right;padding-right:20px;">{{util.money(calc.carprice)}}</y-td>
            </y-tr>
            <!--
            <y-tr v-if="data.estimate.type != 1">
              <y-th>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;시간당 손료 계산</y-th>
              ...
              <y-td style="text-align:right;padding-right:20px;">{{util.money(calc.lossprice)}}</y-td>
            </y-tr>            
            <y-tr v-if="data.estimate.type != 1">
              <y-th>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;주연료</y-th>
              <y-td style="text-align:right;padding-right:10px;">
              ...
              <y-td style="text-align:right;padding-right:20px;">{{util.money(calc.gasprice)}}</y-td>
            </y-tr>
            <y-tr v-if="data.estimate.type != 1">
              <y-th>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;잡품</y-th>              
              <y-td style="text-align:right;padding-right:10px;">                
              ...
              <y-td style="text-align:right;padding-right:20px;">{{util.money(calc.etcprice)}}</y-td>
            </y-tr>
            -->
            <y-tr v-if="data.estimate.type != 1">
              <y-th>&nbsp;&nbsp;&nbsp;위험수당</y-th>              
              ...
              <y-td style="text-align:right;padding-right:20px;">{{util.money(calc.dangerprice)}}</y-td>
            </y-tr>
            <y-tr v-if="data.estimate.type != 1">
              <y-th>&nbsp;&nbsp;&nbsp;기계기구손료</y-th>              
              <y-td style="text-align:right;padding-right:10px;">                
              ...
              <y-td style="text-align:right;padding-right:20px;">{{util.money(calc.machineprice)}}</y-td>
            </y-tr>
            <y-tr v-if="data.estimate.type != 1">
              <y-th>&nbsp;&nbsp;&nbsp;인쇄비</y-th>              
              <y-td style="text-align:right;padding-right:10px;">
              ...
              <y-td style="text-align:right;padding-right:20px;">{{util.money(calc.printprice)}}</y-td>
            </y-tr>
          ...
          <y-table>
            <y-tr>
              <y-th style="width:120px;">합계금액</y-th>
              <y-td></y-td>              
              <y-td style="width:120px;text-align:right;padding-right:20px;">{{util.money(calc.totalprice)}}</y-td>
            </y-tr>
            <y-tr>
              <y-th>절삭</y-th>
              <y-td></y-td>              
              <y-td style="text-align:right;padding-right:10px;">
                <el-input v-model="data.estimate.saleprice"
                          :formatter="(value) => `${value}`.replace(/\D/g,'').replace(/\B(?=(\d{3})+(?!\d))/g, ',')"
                          :parser="(value) => value.replace(/\D/g,'').replace(/\$\s?|(,*)/g, '')"
                          class="inputNumber" style="width:100px;" @input="changeSaleprice" />
              </y-td>
            </y-tr>
            <y-tr>
              <y-th>견적금액</y-th>
              ...
              <y-td style="text-align:right;padding-right:10px;">
                <el-input v-model="data.estimate.price"
                          :formatter="(value) => `${value}`.replace(/\D/g,'').replace(/\B(?=(\d{3})+(?!\d))/g, ',')"
                          :parser="(value) => value.replace(/\D/g,'').replace(/\$\s?|(,*)/g, '')"
                          class="inputNumber" style="width:100px;" />
              </y-td>
            </y-tr>
          </y-table>
        </y-td>
    ...
    <y-table>
      <y-tr>
        ...
        <y-td>
          ...
          <el-checkbox size="small" label="정밀안전진단" v-model="data.c6" style="font-size:12px;" />
...
import { Apt, Category, Repair, Facilitycategory, Standardwage, Contract, Estimate, Inquiry, Periodic } from "~/models"
...
function clickContractInsert() {
  ...
  data.c8 = false
  data.c9 = false  
  data.contract = util.clone(contract)
...
async function clickSubmitEstimate() {
  ...
  if (item.id > 0) {
    await Estimate.update(item)
    util.info('저장되었습니다')
  } else {
    await Estimate.insert(item)
    util.info('등록되었습니다')
  }
...
function changePrice() {
  ...
  if (data.estimate.type != 1) {
    calc.price1 += util.getInt(data.estimate.person6) * data.standardwage.person1
    calc.price2 += util.getInt(data.estimate.person7) * data.standardwage.person2
    calc.price3 += util.getInt(data.estimate.person8) * data.standardwage.person3
    calc.price4 += util.getInt(data.estimate.person9) * data.standardwage.person4
    calc.price5 += util.getInt(data.estimate.person10) * data.standardwage.person5
  }
  ...
  calc.carprice = util.getInt(data.estimate.carprice) * util.getInt(data.estimate.car)
  calc.travelprice = util.getInt(data.estimate.travelprice) * util.getInt(data.estimate.travel)
  ...
  calc.dangerprice = util.getInt(Math.round(util.getFloat(util.getInt(data.estimate.person6) * data.standardwage.person1
                                           + util.getInt(data.estimate.person7) * data.standardwage.person2
                                           + util.getInt(data.estimate.person8) * data.standardwage.person3
                                           + util.getInt(data.estimate.person9) * data.standardwage.person4
                                           + util.getInt(data.estimate.person10) * data.standardwage.person5) * util.getFloat(data.estimate.danger) / 100))  
...
async function clickDownloadEstimate(item) {
  let res = await Apt.get(item.apt)
  let aptItem = res.item
  
  const url = '/api/download/estimate/' + item.id
  let filename

  if (item.type == 1) {
    if (item.subtype == 1) {
      filename = `(주)에이앤비 - ${aptItem.name} - 장기수선계획 조정 견적서.xlsx`
    } else {
      filename = `(주)에이앤비 - ${aptItem.name} - 장기수선계획 수립(조정 포함) 견적서.xlsx`      
    }
  } else if (item.type == 2) {
    filename = `(주)에이앤비 - ${aptItem.name} - 정밀안전점검 견적서.xlsx`
  } else {
    filename = `(주)에이앤비 - ${aptItem.name} - 정기안전점검 견적서.xlsx`
  }
